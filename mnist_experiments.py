...
#A class for Linear Regression Models
    ...
    def fit(self, X, y):
        ...
        if self.method == 'gd':
            ...
            labels.append('gradient descent')
            #start the timer to calculate running time
            t0 = time.clock()
            ...
            for i in range(self.num_iter):
                ...
                #when plotting running time:
                #x_vals.append((time.clock() - t0))
                #when plotting iterations, x_vals.append(i)
                if self.test_method == 'i':
                    x_vals.append(i)
                else:
                    x_vals.append((time.clock() - t0))
                y_vals.append(mean)
        ...
        if self.method == 'sgd':
            ...
            #set labels for plotting
            labels.append('stochastic gradient descent')
            t1 = time.clock()
            ...
            for i in range(self.num_iter):              
                ...
                if self.test_method == 'i':
                    ...
                    x_vals_s.append((time.clock() - t1))
                y_vals_s.append(mean)                
                if i >= 100:
                    time_1 = 1/(d)
                    sum_3 = np.sum(abs(self.weight-w_history[i-100,:]))
        ...
        #When running test 3 and test 4
        #These 5 lines have to be commented
        
        if self.test_1_2 == True:
            if self.test_method == 'r':
                plt.suptitle("Loss function versus Time")
            else:
                plt.suptitle("Loss function versus Iterations")
            
            if self.method == 'gd':
                plt.plot(x_vals,y_vals)
            else:
                plt.plot(x_vals_s,y_vals_s)
...
def main():
    ...
    labels = []
    #set iter to 1000
    model_sgd = MyLogisticReg(w, 500,'sgd',0.01,'i',True)

    model_sgd.fit(X,Y)
    ypred = model_sgd.predict(X)
    a = evaluate(Y,ypred)
  
    model_gd = MyLogisticReg(w, 500,'gd',0.01,'i',True)
    model_gd.fit(X,Y)
    ypred = model_gd.predict(X)
    b = evaluate(Y,ypred)     
    ...
    print('Stochastic gradient_descent accuracy: ',a)
    print('gradient_descent accuracy: ',b)    
    ...
    w_2 = []
    labels = []    
    model_sgd_2 = MyLogisticReg(w_2, 500,'sgd',0.01,'r',True)
    model_sgd_2.fit(X,Y)
    ypred_2 = model_sgd_2.predict(X)
    a_2 = evaluate(Y,ypred_2)
    
    
    model_gd_2 = MyLogisticReg(w_2, 500,'gd',0.01,'r',True)
    model_gd_2.fit(X,Y)
    ypred_2_2 = model_gd_2.predict(X)
    b_2 = evaluate(Y,ypred_2_2)    
    ...
    print('Stochastic gradient_descent accuracy: ',a_2)
    print('gradient_descent accuracy: ',b_2)    
    ...
    for i in range(0,len(lambda_set)):
        model_sgd = MyLogisticReg(w, 500,'gd',lambda_set[i])
    ...
    for i in range(0,len(lambda_set)):
        model_sgd_new = MyLogisticReg(w, 500,'gd',lambda_set[i])
    ...
    for i in range(0,len(w_by_w)):
        model_4 = MyLogisticReg(w_by_w[i],500,'gd',0.01,'r')
        model_4.fit(X,Y)
